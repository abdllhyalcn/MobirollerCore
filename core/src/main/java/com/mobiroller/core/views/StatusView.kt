...
class StatusView @JvmOverloads constructor(
    ...
    private lateinit var mTextPaintStatusIncomplete: TextPaint
    private lateinit var mTextPaintStatusCurrent: TextPaint
...
//            require(value in 0..1) { "Zoom should be in between 0 to 1, but was $value." }
    ...
    */
    ...
    var textColorStatus: Int by OnValidateProp(Color.BLACK) {
        ...
        statusData.forEachIndexed { index, item ->
            if (index <= currentCountIndex()) {
                item.staticLayout?.paint?.color = textColorStatus
            } else {
                item.staticLayout?.paint?.color = textColorStatusIncomplete
            }
        }
...
//        }
    ...
    /**
     * Text Color of status  Incomplete
     */
    var textColorStatusIncomplete: Int by OnValidateProp(Color.BLACK) {
        mTextPaintStatusIncomplete.color = textColorStatusIncomplete
    }

    /**
     * Text Color of status  Current
     */
    var textColorStatusCurrent: Int by OnValidateProp(Color.BLACK) {
        mTextPaintStatusCurrent.color = textColorStatusCurrent
    }
    /**
    ...
    var textSizeStatus: Float by OnLayoutProp(14.0f.pxValue(TypedValue.COMPLEX_UNIT_SP)) {
        ...
        mTextPaintStatusIncomplete.textSize = textSizeStatus
        mTextPaintStatusCurrent.textSize = textSizeStatus
    }
    ...
    var statusTypeface: Typeface? by OnLayoutProp(null) {
        statusTypeface?.run {
            ...
            mTextPaintStatusIncomplete.typeface = this
            mTextPaintStatusCurrent.typeface = this
        }
    ...
    private class StatusItemText(val x: Float = 0.0f, val y: Float = 0.0f, val paint: TextPaint? = null, val staticLayout: StaticLayout? = null)
    ...
    init {
        ...
        try {
            ...
            textColorStatusIncomplete = a.getColor(R.styleable.StatusViewScroller_textColorStatusIncomplete, textColorLabels)
            textColorStatusCurrent = a.getColor(R.styleable.StatusViewScroller_textColorStatusCurrent, textColorLabelsIncomplete)
        ...
        mTextPaintStatusCurrent = TextPaint(Paint.ANTI_ALIAS_FLAG)
        mTextPaintStatusCurrent.style = Paint.Style.FILL
        mTextPaintStatusCurrent.textAlign = Paint.Align.CENTER
        mTextPaintStatusCurrent.color = textColorStatusCurrent
        mTextPaintStatusCurrent.textSize = textSizeStatus
        statusTypeface?.run {
            mTextPaintStatusCurrent.typeface = this
        ...
        mTextPaintStatusIncomplete = TextPaint(Paint.ANTI_ALIAS_FLAG)
        mTextPaintStatusIncomplete.style = Paint.Style.FILL
        mTextPaintStatusIncomplete.textAlign = Paint.Align.CENTER
        mTextPaintStatusIncomplete.color = textColorStatusIncomplete
        mTextPaintStatusIncomplete.textSize = textSizeStatus
        statusTypeface?.run {
            mTextPaintStatusIncomplete.typeface = this
        ...
        for (item in drawingData) {
            ...
            if (item.labelData != null) {
                ...
                if (item.labelData.staticLayout != null) {
                    item.labelData.staticLayout.paint.set(item.labelData.paint)
                    canvas?.save()
    ...
    private fun setDrawingDimensions() {
        ...
        for (pos in 0 until stepCount) {
            ...
            if (isShowingCurrentStatus() && pos == (currentCount - 1)) {
                ...
                textPaintStatus = mTextPaintStatusCurrent
                ...
                textPaintStatus = mTextPaintStatusIncomplete
            ...
            if (pos < statusData.size) {
                ...
                labelItemText = StatusItemText(circleItem.center.x, circleItem.center.y + radii + circleStrokeWidth / 2 + statusTopMargin, textPaintStatus ,statusData[pos].staticLayout)
            }
