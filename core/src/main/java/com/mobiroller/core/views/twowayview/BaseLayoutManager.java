...
import static com.mobiroller.core.views.twowayview.Lanes.calculateLaneSize;
...
public abstract class BaseLayoutManager extends TwoWayLayoutManager {
    ...
    protected static class ItemEntry implements Parcelable {
        ...
        public static final Creator<BaseLayoutManager.ItemEntry> CREATOR
                = new Creator<BaseLayoutManager.ItemEntry>() {
            @Override
            public BaseLayoutManager.ItemEntry createFromParcel(Parcel in) {
                return new BaseLayoutManager.ItemEntry(in);
            }

            @Override
            public BaseLayoutManager.ItemEntry[] newArray(int size) {
                return new BaseLayoutManager.ItemEntry[size];
            }
        };
    }
    ...
    protected void pushChildFrame(BaseLayoutManager.ItemEntry entry, Rect childFrame, int lane, int laneSpan,
                                  Direction direction) {
    ...
    private void popChildFrame(BaseLayoutManager.ItemEntry entry, Rect childFrame, int lane, int laneSpan,
                               Direction direction) {
        for (int i = lane; i < lane + laneSpan; i++) {
            final int spanMargin;
            if (entry != null && direction != Direction.END) {
    ...
    void setItemEntryForPosition(int position, BaseLayoutManager.ItemEntry entry) {
        if (mItemEntries != null) {
            mItemEntries.putItemEntry(position, entry);
        }
    }

    BaseLayoutManager.ItemEntry getItemEntryForPosition(int position) {
    ...
    private void handleUpdate(int positionStart, int itemCountOrToPosition, BaseLayoutManager.UpdateOp cmd) {
    ...
    public void onItemsAdded(RecyclerView recyclerView, int positionStart, int itemCount) {
        handleUpdate(positionStart, itemCount, BaseLayoutManager.UpdateOp.ADD);
        super.onItemsAdded(recyclerView, positionStart, itemCount);
    }

    @Override
    public void onItemsRemoved(RecyclerView recyclerView, int positionStart, int itemCount) {
        handleUpdate(positionStart, itemCount, BaseLayoutManager.UpdateOp.REMOVE);
        super.onItemsRemoved(recyclerView, positionStart, itemCount);
    }

    @Override
    public void onItemsUpdated(RecyclerView recyclerView, int positionStart, int itemCount) {
        handleUpdate(positionStart, itemCount, BaseLayoutManager.UpdateOp.UPDATE);
        super.onItemsUpdated(recyclerView, positionStart, itemCount);
    }

    @Override
    public void onItemsMoved(RecyclerView recyclerView, int from, int to, int itemCount) {
        handleUpdate(from, to, BaseLayoutManager.UpdateOp.MOVE);
    ...
    public Parcelable onSaveInstanceState() {
        ...
        final BaseLayoutManager.LanedSavedState state = new BaseLayoutManager.LanedSavedState(superState);
    ...
    public void onRestoreInstanceState(Parcelable state) {
        final BaseLayoutManager.LanedSavedState ss = (BaseLayoutManager.LanedSavedState) state;
    ...
    protected void layoutChild(View child, Direction direction) {
        ...
        final BaseLayoutManager.ItemEntry entry = cacheChildFrame(child, mChildFrame);
    ...
    BaseLayoutManager.ItemEntry cacheChildLaneAndSpan(View child, Direction direction) {
        // Do nothing by default.
        return null;
    }

    BaseLayoutManager.ItemEntry cacheChildFrame(View child, Rect childFrame) {
        // Do nothing by default.
        return null;
    }
    ...
    protected static class LanedSavedState extends SavedState {
        ...
        private LanedSavedState(Parcel in) {
            ...
            if (itemEntriesCount > 0) {
                ...
                for (int i = 0; i < itemEntriesCount; i++) {
                    final BaseLayoutManager.ItemEntry entry = in.readParcelable(getClass().getClassLoader());
        ...
        public static final Parcelable.Creator<BaseLayoutManager.LanedSavedState> CREATOR
                = new Parcelable.Creator<BaseLayoutManager.LanedSavedState>() {
            @Override
            public BaseLayoutManager.LanedSavedState createFromParcel(Parcel in) {
                return new BaseLayoutManager.LanedSavedState(in);
            }

            @Override
            public BaseLayoutManager.LanedSavedState[] newArray(int size) {
                return new BaseLayoutManager.LanedSavedState[size];
            }
        };
    }
}
