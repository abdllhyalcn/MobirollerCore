package com.mobiroller.core.views.twowayview;

/**
 * Created by ealtaca on 14.02.2017.
 */
...
public abstract class TwoWayLayoutManager extends LayoutManager {
    ...
    private TwoWayLayoutManager.SavedState mPendingSavedState = null;
    ...
    public TwoWayLayoutManager(Context context, AttributeSet attrs, int defStyle) {
        ...
        for (int i = 0; i < indexCount; i++) {
            ...
            if (attr == R.styleable.twowayview_TwoWayLayoutManager_android_orientation) {
                ...
                if (orientation >= 0) {
                    setOrientation(TwoWayLayoutManager.Orientation.values()[orientation]);
                }
            }
        }
    ...
    public TwoWayLayoutManager(TwoWayLayoutManager.Orientation orientation) {
    ...
    private void recycleChildrenOutOfBounds(TwoWayLayoutManager.Direction direction, Recycler recycler) {
    ...
    private void recycleChildrenFromStart(TwoWayLayoutManager.Direction direction, Recycler recycler) {
    ...
    private void recycleChildrenFromEnd(TwoWayLayoutManager.Direction direction, Recycler recycler) {
    ...
    private int scrollBy(int delta, Recycler recycler, State state) {
        ...
        final TwoWayLayoutManager.Direction direction = (delta > 0 ? TwoWayLayoutManager.Direction.END : TwoWayLayoutManager.Direction.START);
        recycleChildrenOutOfBounds(direction, recycler);

        final int absDelta = Math.abs(delta);
        if (canAddMoreViews(TwoWayLayoutManager.Direction.START, start - absDelta) ||
                canAddMoreViews(TwoWayLayoutManager.Direction.END, end + absDelta)) {
    ...
    private void fillGap(TwoWayLayoutManager.Direction direction, Recycler recycler, State state) {
        final int childCount = getChildCount();
    ...
    private void fillBefore(int position, Recycler recycler, int extraSpace) {
        ...
        while (canAddMoreViews(TwoWayLayoutManager.Direction.START, limit) && position >= 0) {
            makeAndAddView(position, TwoWayLayoutManager.Direction.START, recycler);
            position--;
        }
    }
    ...
    private void fillAfter(int position, Recycler recycler, State state, int extraSpace) {
        ...
        while (canAddMoreViews(TwoWayLayoutManager.Direction.END, limit) && position < itemCount) {
            makeAndAddView(position, TwoWayLayoutManager.Direction.END, recycler);
            position++;
        }
    }
    ...
    private static View findNextScrapView(List<ViewHolder> scrapList, TwoWayLayoutManager.Direction direction,
                                          int position) {
        ...
        for (int i = 0; i < scrapCount; i++) {
            ...
            if ((distance < 0 && direction == TwoWayLayoutManager.Direction.END) ||
                    (distance > 0 && direction == TwoWayLayoutManager.Direction.START)) {
                continue;
            }
    ...
    private void fillFromScrapList(List<ViewHolder> scrapList, TwoWayLayoutManager.Direction direction) {
        final int firstPosition = getFirstVisiblePosition();
        ...
        while ((scrapChild = findNextScrapView(scrapList, direction, position)) != null) {
            ...
            position += (direction == TwoWayLayoutManager.Direction.END ? 1 : -1);
        }
    }

    private void setupChild(View child, TwoWayLayoutManager.Direction direction) {
    ...
    private View makeAndAddView(int position, TwoWayLayoutManager.Direction direction, Recycler recycler) {
        final View child = recycler.getViewForPosition(position);
        final boolean isItemRemoved = ((LayoutParams) child.getLayoutParams()).isItemRemoved();

        if (!isItemRemoved) {
            addView(child, (direction == TwoWayLayoutManager.Direction.END ? -1 : 0));
        }
    ...
    private void updateLayoutEdgesFromRemovedChild(View removedChild, TwoWayLayoutManager.Direction direction) {
        final int childCount = getChildCount();
        if (childCount == 0) {
            resetLayoutEdges();
            return;
    ...
    protected void onLayoutScrapList(Recycler recycler, State state) {
        ...
        fillFromScrapList(scrapList, TwoWayLayoutManager.Direction.START);
        fillFromScrapList(scrapList, TwoWayLayoutManager.Direction.END);
    }

    protected void detachChild(View child, TwoWayLayoutManager.Direction direction) {
    ...
    public Parcelable onSaveInstanceState() {
        final TwoWayLayoutManager.SavedState state = new TwoWayLayoutManager.SavedState(TwoWayLayoutManager.SavedState.EMPTY_STATE);
    ...
    public void onRestoreInstanceState(Parcelable state) {
        mPendingSavedState = (TwoWayLayoutManager.SavedState) state;
        requestLayout();
    }
    ...
    public void setOrientation(TwoWayLayoutManager.Orientation orientation) {
        final boolean isVertical = (orientation == TwoWayLayoutManager.Orientation.VERTICAL);
    ...
    protected abstract void measureChild(View child, TwoWayLayoutManager.Direction direction);
    protected abstract void layoutChild(View child, TwoWayLayoutManager.Direction direction);

    protected abstract boolean canAddMoreViews(TwoWayLayoutManager.Direction direction, int limit);

    protected static class SavedState implements Parcelable {
        protected static final TwoWayLayoutManager.SavedState EMPTY_STATE = new TwoWayLayoutManager.SavedState();
        ...
        public static final Parcelable.Creator<TwoWayLayoutManager.SavedState> CREATOR
                = new Parcelable.Creator<TwoWayLayoutManager.SavedState>() {
            @Override
            public TwoWayLayoutManager.SavedState createFromParcel(Parcel in) {
                return new TwoWayLayoutManager.SavedState(in);
            }

            @Override
            public TwoWayLayoutManager.SavedState[] newArray(int size) {
                return new TwoWayLayoutManager.SavedState[size];
            }
        };
    }
}
